...
- Name: settings.spec.ts >> Settings Page >> should display settings heading
- Location: e2e/settings.spec.ts:9:7
...
Locator: locator('h1, h2').filter({ hasText: 'Settings' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('h1, h2').filter({ hasText: 'Settings' })
