...
- Name: landing.spec.ts >> Landing Page >> should display hero section with title
...
Locator: locator('text=dMessage')
Expected: visible
Error: strict mode violation: locator('text=dMessage') resolved to 5 elements:
    1) <span class="font-mono text-lg font-black tracking-tight text-white">dMessage</span> aka getByRole('link', { name: 'dMessage' })
    2) <h1 class="font-mono text-[var(--text-hero)] font-black leading-none tracking-tighter text-white">dMessage</h1> aka getByRole('heading', { name: 'dMessage' })
    3) <span class="font-mono text-xs uppercase tracking-[0.3em] text-[var(--text-muted)]">Why dMessage</span> aka getByText('Why dMessage')
    4) <span class="font-mono text-lg font-black tracking-tight text-white">dMessage</span> aka locator('footer').getByText('dMessage', { exact: true })
    5) <p class="font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--text-faint)]">…</p> aka getByText('© 2026 dMessage · MIT License')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('text=dMessage')
