...
- Name: landing.spec.ts >> Landing Page >> should display marquee ticker
- Location: e2e/landing.spec.ts:25:7
...
Locator: locator('text=END-TO-END ENCRYPTED')
Expected: visible
Error: strict mode violation: locator('text=END-TO-END ENCRYPTED') resolved to 4 elements:
    1) <span class="mx-6 flex items-center gap-6 font-mono text-xs font-black uppercase tracking-[0.2em]">…</span> aka getByText('END-TO-END ENCRYPTED').first()
    2) <span class="mx-6 flex items-center gap-6 font-mono text-xs font-black uppercase tracking-[0.2em]">…</span> aka getByText('END-TO-END ENCRYPTED').nth(1)
    3) <h3 class="mt-6 font-mono text-base font-black tracking-tight text-white">End-to-End Encrypted</h3> aka getByRole('heading', { name: 'End-to-End Encrypted' })
    4) <p class="mt-4 max-w-xs font-mono text-xs leading-relaxed text-[var(--text-muted)]">Decentralized, end-to-end encrypted messaging on …</p> aka getByText('Decentralized, end-to-end')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('text=END-TO-END ENCRYPTED')
